...
extract_latest_status <- function(eventSummaries) {
  ...
  status <- dplyr::case_when(
    grepl("Pre-Shipment|USPS Awaiting Item|Shipping Label Created|label has been prepared", latest_event) ~ "装箱",
    grepl("USPS picked up item|USPS in possession of item|Departed Post Office", latest_event) ~ "发出",
    grepl("In Transit|Departed|Arrived at|Your item arrived at|Your item departed|is moving|on its way|out for delivery", latest_event) ~ "在途",
