              ...
              div(
                ...
                tags$script(HTML("
                  $(document).ready(function() {
                      ...
                      // 监听输入框内容变化
                      $(document).on('input', '[id$=inbound_sku]', function() {  // 动态匹配 ID
                      ...
                      // 监听回车键事件
                      $(document).on('keydown', '[id$=inbound_sku]', function(e) {  // 使用 keydown 替代 keypress
                          if (e.which === 13) {  // 检测回车键
                              e.preventDefault();  // 阻止默认行为
                  
                              if (isInBoundSkuTypingFinished) {  // 仅在输入完成后允许触发回车
                                  $('[id$=confirm_inbound_btn]').click();  // 动态匹配按钮 ID 并模拟点击
                              }
                          }
                      });
                  });
