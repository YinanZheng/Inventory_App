...
server <- function(input, output, session) {
  ...
  output$download_unique_items_xlsx <- downloadHandler(
    ...
    content = function(file) {
      ...
      # 创建 Excel 文件
      wb <- createWorkbook()
      sheet <- createSheet(wb, sheetName = "物品明细表")
      
      # 写入表头和数据
      addDataFrame(final_data[, -which(colnames(final_data) == "商品图片")], sheet, row.names = FALSE)
      
      # 插入图片
      image_col <- which(colnames(final_data) == "商品图片")  # 商品图片所在列
      if (!is.na(image_col)) {
        for (i in seq_len(nrow(final_data))) {
