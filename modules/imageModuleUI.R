imageModuleUI <- function(id, label = "图片上传区域") {
  ...
  tagList(
    tags$div(
      ...
      # 粘贴区域
      tags$div(
        id = ns("paste_area"), # 模块命名空间下的 ID
        style = "border: 2px dashed #ccc; padding: 20px; text-align: center; margin-bottom: 15px; position: relative;",
        div(id = ns("paste_prompt"), "将图片粘贴到这里（Ctrl+V 或 Cmd+V）", style = "color: #888; font-size: 16px; font-style: italic;"),
        uiOutput(ns("pasted_image_preview")) # 图片预览
      ),
